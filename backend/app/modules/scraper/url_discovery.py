...
class URLDiscovery:
    ...
    async def discover_all(
        ...
        # Fallback: if all scrapers returned empty (likely blocked), log clearly
        if not unique:
            logger.warning(
                "ALL scrapers returned 0 results for '%s' in '%s' from %s. "
                "Likely blocked by Google/Indeed. Try different query or location.",
                role, location, valid_sources,
            )
